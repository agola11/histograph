(function ($) {
    ...
    $.fn.extend({
        sliding_menu_js: function (opciones) {
            ...
            // $('#leftmenu ul').append($(this).html());

            // Se eliminan elementos innecesarios
            // $('.divider').remove();
            // $('#leftmenu ul').removeClass();
            // $('#leftmenu ul li').removeClass();
            // $('#leftmenu ul li a').removeClass();

            // Titulo
            // if (config.header_title) {
            //     $('#leftmenu .header').prepend("<h3>" + config.header_title + "</h3>");
            //     // $('#sliding_menu_js_btn').append("<h3>" + config.header_title + "</h3>");
            // }

            // Logo
            // if (config.header_logo) {
            //     $('#leftmenu .header').prepend("<img src='" + config.header_logo + "' />");
            // }

            // Transición
            $('#leftmenu').css('transition', 'left ' + config.transitionSpeed + 's ' + config.easing);
            ...
            function mostrar(){
                if ( $('#leftmenu').hasClass('cerrado') ) {
                    $('#leftmenu').css('left','0px');
                    $('#leftmenu').removeClass('cerrado');
